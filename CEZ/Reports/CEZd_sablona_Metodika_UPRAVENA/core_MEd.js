...
function core_ME(oModel) {
    ...
    var nazevDokumentu = oModel.Attribute(Constants.AT_NAME, g_Loc).getValue() //getNameOfDoc(oModel);
    var zpracovatel = oModel.Attribute(attr_zpracovatel, g_Loc).getValue();
    var processModel = getProcess(oModel.SuperiorObjDefs()); //superior object model
    var process = processModel;

    function getProcess(superiorObjDefs) {

        var modelFuncTree = getFuncTree(superiorObjDefs)

        if (superiorObjDefs.length > 0)
            var occ = superiorObjDefs[0].OccListInModel(modelFuncTree)
        else
            return "";

        var superiorOccs = occ[0].getConnectedObjOccs([Constants.ST_FUNC], Constants.EDGES_IN)
        if (superiorOccs.length > 0) {
            var assignModels = superiorOccs[0].ObjDef().AssignedModels();
            if (assignModels.length > 0)
                return assignModels[0].Name(g_Loc);
            else return ""
        } else return "";

        function getFuncTree(superiorObjDefs) {
            for (var i = 0; i < superiorObjDefs.length; i++) {
                //proces = superiorObjDefs[i].Name(g_Loc);
                var occs = superiorObjDefs[i].OccList()
                for (var j = 0; j < occs.length; j++) {
                    if (occs[j].Model().TypeNum() == Constants.MT_FUNC_TREE)
                        return occs[j].Model();
                }
            }
        }
    ...
    replaceTextInWordDOC("ARIS_PROCES", process, "Text_Left_BOLD");
    replaceTextInWordDOC("ARIS_GARANT", "", "Text_Left_BOLD");
    // replaceTextInWordDOC("ARIS_ZPRACOVATEL", "", "Text_Left_BOLD");
    replaceTextInWordDOC("ARIS_PROCES", process, "Text_Left");
    /* Content */
...
*/
    ...
    var oRolesDef = getRoleObjectsFromLane(oModel); //retunr all defs from first column

    var arr_ARIS_GENERALNI_REDITEL = new Array();
    var arr_FINANCE_A_SPRAVA = new Array();
    var arr_ARIS_RIZENI_DIS_AKTIV = new Array();
    var arr_SITOVE_SLUZBY = new Array();
    var arr_PROVOZ_A_RIZENI_DS = new Array();


    for (var i = 0; i < oRolesDef.length; i++) {
        var cislo = null;
        var sname = oRolesDef[i].Attribute(Constants.AT_NAME, g_Loc).getValue();
        switch (oRolesDef[i].TypeNum()) {
            case Constants.OT_POS:
                if(oRolesDef[i].Attribute(attr_cisloPM, g_Loc).IsMaintained())
                    cislo = oRolesDef[i].Attribute(attr_cisloPM, g_Loc).getValue();
                break;
            case Constants.OT_ORG_UNIT:
                if(oRolesDef[i].Attribute(attr_cisloUtvaru, g_Loc).IsMaintained())
                    cislo = oRolesDef[i].Attribute(attr_cisloUtvaru, g_Loc).getValue();
                break;
            case Constants.OT_POS:
                if(oRolesDef[i].Attribute(attr_cisloPM, g_Loc).IsMaintained())
                    cislo = oRolesDef[i].Attribute(attr_cisloPM, g_Loc).getValue();
            ...
            case Constants.OT_PERS_TYPE:
                if(oRolesDef[i].Attribute(attr_cisloUtvaruPref, g_Loc).IsMaintained())
                    cislo = oRolesDef[i].Attribute(attr_cisloUtvaruPref, g_Loc).getValue();
                break;
        ...
        if (cislo != null) {
            var  arr_cislo = new Array();
             if(cislo.indexOf(";")>0)
                 arr_cislo = cislo.split(";")
            if(cislo.indexOf(";")== -1)
                arr_cislo = arr_cislo.push(cislo)
            
            
            for (var j = 0; j < arr_cislo.length; j++) {
                var start = arr_cislo[j].indexOf("_")+1;
                 if(start>= 0)
                    var symbol = arr_cislo[j].substring(start, start+1);
                    var trimCislo = arr_cislo[j].substring(start, arr_cislo[j].length)
              
              
                switch (symbol) {
                    case "1":
                        arr_ARIS_GENERALNI_REDITEL.push(trimCislo); //Úsek Generální ředitel
                        break;
                    case "3":
                arr_FINANCE_A_SPRAVA.push(trimCislo); //Úsek Finance a správa
                        break;
                    case "9":
                        arr_ARIS_RIZENI_DIS_AKTIV.push(trimCislo); //Úsek Řízení distribučních aktiv
                        break;
                    case "A":
                        arr_SITOVE_SLUZBY.push(trimCislo); //Úsek Síťové služby
                        break;
                    case "B":
                        arr_PROVOZ_A_RIZENI_DS.push(trimCislo); //Úsek Provoz a řízení DS
                        break;
                    default:
                }
            }
    ...
    replaceTextInWordDOC("ARIS_GENERALNI_REDITEL", printArry(arr_ARIS_GENERALNI_REDITEL.sort()), getString("TEXT_STYLE_DEFAULT"));
    replaceTextInWordDOC("ARIS_FINANCE_A_SPRAVA", printArry(arr_FINANCE_A_SPRAVA.sort()), getString("TEXT_STYLE_DEFAULT"));
    replaceTextInWordDOC("ARIS_RIZENI_DIS_AKTIV", printArry(arr_ARIS_RIZENI_DIS_AKTIV.sort()), getString("TEXT_STYLE_DEFAULT"));
    replaceTextInWordDOC("ARIS_SITOVE_SLUZBY", printArry(arr_SITOVE_SLUZBY.sort()), getString("TEXT_STYLE_DEFAULT"));
    replaceTextInWordDOC("ARIS_PROVOZ_A_RIZENI_DS", printArry(arr_PROVOZ_A_RIZENI_DS.sort()), getString("TEXT_STYLE_DEFAULT"));
    ...
    replaceTextInWordDOC("ARIS_NAZEV_PROCESU", nazevDokumentu, getString("TEXT_STYLE_DEFAULT"));

    //optionaly insert model
    var replace = g_oOut.SetPositionToField("ARIS_NAHLED_MODELU", true); //for section 2
    if (replace) {
        insertModelPic(oModel)
        g_oOut.OutputLnF("", getString("TEXT_STYLE_DEFAULT"));
    }


    // replaceTextInWordDOC("ARIS_NAHLED_MODELU",    insertModelPic(oModel), "Text_Left_BOLD");
    // g_oOut.OutputLnF("", getString("TEXT_STYLE_DEFAULT"))
    ...
    if (replace) {
        ...
        for (var k = 0; k < aObjFuncsInEPC.length; k++) {
            if (aObjFuncsInEPC[k].SymbolNum() == Constants.ST_FUNC || aObjFuncsInEPC[k].SymbolNum() == Constants.ST_SYS_FUNC_ACT) //335 only fun with green symbols
            {
                ...
                insertMark_forHeader( /*keyIndex +*/ cinnost, aObjFuncsInEPC[k].ObjDef().GUID(), getString("TEXT_STYLE_HEADER2_HIDDEN"), getString("TEXT_STYLE_HEADER2"));
                ...
                if (podleCeho_EPC.length > 0) {  //RV 20220503
                    g_oOut.TableRow();
                    g_oOut.TableCellF("Podle čeho", 20, "Normal_Text");
                    printRowDesc(podleCeho_EPC, 80);
                }
                g_oOut.TableRow();
