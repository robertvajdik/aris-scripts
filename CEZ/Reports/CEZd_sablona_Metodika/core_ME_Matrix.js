...
function outTables(arrOuter, sProcessName, bCheckFuncAssignments) {
    ...
    while (bLoop) {
        ...
        if (bCheckFuncAssignments) {
            /***********/
            g_oOut.TableCell(getString("TEXT_2"), 20, getString("TEXT_3"), 8, Constants.C_BLACK, new java.awt.Color(0.65625, 0.8984375, 0.57421875).getRGB(), 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VTOP, 0);
            /*************/
        }
       ...
       //g_oOut.SetFrameStyle(Constants.FRAME_TOP, 10) // 0: zeichnet z.B. keine */
        ...
        g_oOut.TableCell("Funkce↓/Role→", 22, getString("TEXT_3"), 8, Constants.C_BLACK, Constants.C_WHITE, 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VCENTER, 0);

       
        for (var j = nStart; j < nEnd; j++) {
             g_oOut.TableCell(arrOuter[0][j].roleElemDef.Name(g_Loc), 6, getString("TEXT_3"), 8, Constants.C_BLACK, RGB(255, 255, 255), 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VERT_UP, 0);
            /*
            if (arrOuter[0][j].roleElemDef.TypeNum() == Constants.OT_APPL_SYS_TYPE)
                g_oOut.TableCell(arrOuter[0][j].roleElemDef.Name(g_Loc), 6, getString("TEXT_3"), 8, Constants.C_BLACK, RGB(150, 255, 255), 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VERT_UP, 0);
            else
                g_oOut.TableCell(arrOuter[0][j].roleElemDef.Name(g_Loc), 6, getString("TEXT_3"), 8, Constants.C_BLACK, RGB(255, 255, 150), 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VERT_UP, 0);
            */
        }
         g_oOut.ResetFrameStyle();
        ...
        for (var i = 0; i < arrOuter.length; i++) {
          //  if (hasFunctionAworth(arrOuter[i]) == true) {
                ...
                if (bCheckFuncAssignments) {
                    g_oOut.TableCell(getProcessAssignment(arrOuter[i][0].funcDef), 22, getString("TEXT_3"), 8, Constants.C_BLACK, RGB(149, 259, 149), 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
                }
                g_oOut.ResetFrameStyle();
                g_oOut.SetFrameStyle(Constants.FRAME_LEFT, 0) // 0: zeichnet z.B. keine */
             ...
             //   if (arrOuter[i][0].funcDef.Attribute(1208, g_Loc).IsMaintained())
                 ...
                 g_oOut.TableCell(keyIndex + arrOuter[i][0].funcDef.Name(g_Loc), 22, getString("TEXT_3"), 8, Constants.C_BLACK, RGB(255, 255, 255), 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
                //g_oOut.TableCell(keyIndex + arrOuter[i][0].funcDef.Name(g_Loc), 22, getString("TEXT_3"), 8, Constants.C_BLACK, RGB(150, 255, 150), 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);

                 g_oOut.ResetFrameStyle();
                for (var j = nStart; j < nEnd; j++) {
                    g_oOut.TableCell(arrOuter[i][j].rasciValue, 6, getString("TEXT_3"), 8, Constants.C_BLACK, arrOuter[i][j].colour, 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VCENTER, 0);
                }
         /*   } else {
                if (bCheckFuncAssignments) {
        ...
        g_oOut.EndTable("", 100, getString("TEXT_3"), 10, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
...
function outNotAssignedFuncsList() {
    ...
    for (var iter = g_aNotAssignedFuncs.iterator(); iter.hasNext();) {
        var func = iter.next();
        g_oOut.TableRow();
        g_oOut.TableCell(func.Name(g_Loc), 20, getString("TEXT_3"), 10, Constants.C_BLACK, new java.awt.Color(0.72265625, 0.99609375, 0.72265625).getRGB(), 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VERT_DOWN, 0);
        g_oOut.TableCell(getProcessAssignment(func), 20, getString("TEXT_3"), 10, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_BOLD | Constants.FMT_CENTER | Constants.FMT_VTOP, 0);
    }
    g_oOut.EndTable(getString("TEXT_4"), 100, getString("TEXT_3"), 10, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
}
