...
function writeRisksIntoModel(model) {
    ...
    try {
        ...
        model.ObjOccListFilter(Constants.OT_RISK)
             .forEach(function (r) {
                 r.Remove();
             });
        ...
        var statusObject = model.ObjOccListFilter(-1, SYM_STAT)
        if (statusObject.length === 0) return;

        var occurrences = model.ObjOccListFilter(OBJ_DOC)
...
function formatModel(model) {
    ...
    try {
        ...
        writeLog(sm72tc,"During formatting model " + model.Group().Path(g_nLoc)+"\\"+model.Name(g_nLoc) + " error occurred " + e,"info" );
    }
}
    ...
    switch (Action) {
        case 1: // Dialogbox-Initialisierung
            ...
            if (
                ...
                var array = String(g_tMainObject.Risks.sAttr_Risk).split("|")
                                                                  .map(function (item) {
...
function RiskManagementAddDialog(model, assignedRisks, corporateKey, locale) {
    ...
    this.onClose = function (pageNumber, bOk) {
        ...
        this.dialogResult.selectedOption = this.getPageElement(0, "RISK_MANAGEMENT_RISK_TYPE_SELECTOR").getSelectedIndex();
    ...
    return String(input).split("|")
                        ...
                        .map(function (json) {
                            if (json.riskType !== RISK_OPTION_3) {
                                ...
                                var risks = document.getConnectedObjs(Constants.OT_RISK)
...
RiskUtils.isInModel = function (model, object) {
    ...
    return model.ObjOccListFilter(Constants.OT_INFO_CARR)
                .filter(function (occurrence) {
